...
const ProductPage = () => {
  const { id } = useParams();
  const { data } = useFetch(`/products/${id}.json`);
  const addingToCart = () => {
    dispatch(addToCart(data));
  };
  const dispatch = useDispatch();
  const products = useSelector((state) => state.cart.products);
  const existingItem = !!products.find((item) => item.id === parseInt(id, 10));
            ...
            <div className="mt-6 w-full lg:mt-0 lg:w-1/2 lg:py-6 lg:pl-10">
              ...
              <div className="mb-5 mt-6 flex items-center border-b-2 border-gray-200 pb-5">
                <div className="flex">
                  ...
                  <button className="#A36B5E ml-1 h-6 w-6 rounded-full border-2 border-gray-300 focus:outline-none"></button>
                </div>
              </div>
              <div className="flex">
